...
def insert_raw_data():
  ...
  df.to_sql(name='khairul_raw_covid', con=engine, if_exists="replace", index=False)
  engine.dispose()
...
def insert_raw_to_warehouse(schema):
    mysql_auth = MySQL(credential['mysql_lake'])
    engine, engine_conn = mysql_auth.connect()
    data = pd.read_sql(sql='khairul_raw_covid', con=engine)
    engine.dispose()
...
if __name__ == '__main__':
  insert_raw_data()
  create_star_schema(schema='khairul')
  insert_raw_to_warehouse(schema='khairul')